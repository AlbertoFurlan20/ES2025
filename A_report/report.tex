...
Delivering that required four pieces rather than one, because the BSP does not
provide everything the driver needs:
...
\texttt{telemetry/} & \texttt{C\_src/src/telemetry/} & measurement stream over USART2, for host-side analysis \\
\end{tabular}
\end{center}
...
numbers are derived on the host, where they can be recomputed without
reflashing.
...
\item \emph{The driver model.} The driver plugs into the RTEMS generic I2C
  ...
  destroy handler, not the caller, is what frees it. Getting that ownership
  backwards is a use-after-free, and we hit exactly that (Section
  \ref{sec:problems}).
...
\item \emph{Scheduling and priority.} The final design is two tasks at fixed
  priorities, and the priority \emph{order} is load-bearing rather than
  incidental: the consumer must sit \emph{below} the producer. We measured a
  2.70~s console blackout caused by getting the interaction wrong --- a
  priority-2 task busy-looping on a failing ioctl starved every task below it.
\item \emph{Concurrency without blocking the real-time path.} The application
  layer publishes measurements through a seqlock rather than a mutex,
  specifically so that a reader can never delay acquisition.
\item \emph{Bounded blocking and fault containment.} Every I2C poll has a 5~ms
  ...
  system freeze.
\item \emph{Timing as a measurable property.} Because RTEMS
  ...
  specification. That single tick of rounding was a real, stochastic defect in
  our first release, visible only in the noise statistics.
\end{itemize}
...
configured clock. A line shorted to ground cannot be freed this way and is
reported as such --- an interrupted transfer is recoverable, a shorted line is
...
(\texttt{0xD0}, expecting \texttt{0x55}) and publishes the node. That first I2C
traffic is deliberately minimal: a registration that succeeds proves the wiring,
...
\texttt{bmp180\_unregister()} goes through \texttt{unlink()}, which is the only
correct order, since the IMFS node owns the device.
...
Two design decisions inside the driver are worth naming:
...
\paragraph{The application layer (\texttt{bmp\_app/}).} A driver that only one
task can use is not much of a driver. \texttt{bmp\_app\_sampler\_task} is the
only thing in the system that opens \texttt{/dev/bmp180-0}: it acquires
back-to-back and publishes each measurement into a one-deep snapshot. Consumers
call \texttt{bmp\_app\_read()}, which never blocks and adds no bus traffic, and
change the mode or the temperature interval through setters the sampler applies
at the top of its next cycle. That indirection is the point --- \texttt{oss} has
exactly one writer, so two consumers cannot fight over the mode, and no consumer
holds a descriptor on the device.
...
The layer is honest about staleness rather than hiding it: \emph{a successful
read does not mean a fresh reading}. A failed cycle does not publish, so a
sensor that stops answering leaves the last good sample --- with its last good
timestamp --- in place indefinitely. The alternative, stamping stale values with
a fresh timestamp, is precisely the class of defect we had to fix earlier.
Consumers detect it by comparing \texttt{seq} against their previous read, or
...
\texttt{bmp\_app\_set\_oss()}, and writes records to USART2. The wire format is
frozen at v1 and is deliberately machine-readable rather than human prose:
...
records. Errors are emitted on the edge --- a disconnected sensor fails hundreds
of times a second and would otherwise flood the console.

\paragraph{Tasks and priorities.} The image runs two tasks after
initialisation:
...
firmware then registers the bus, registers the sensor, and starts streaming.
...
\url{https://github.com/AlbertoFurlan20/ES2025}, 53 commits and ten firmware
releases from 1.0.0 to \textbf{2.0.0} (tags \texttt{v1.0.0} through
\texttt{v1.5.0}). \texttt{CHANGELOG.md} records
every release, what changed in the measurement path and what was measured to
justify it, which is also the honest record of the defects listed in Section
\ref{sec:problems}.
...
Telemetry & \texttt{C\_src/src/telemetry/} & 268 lines \\
Boot and RTEMS config & \texttt{C\_src/src/init.cpp} & 139 lines \\
Public headers & \texttt{C\_src/inc/} & 11 headers \\
\end{tabular}
\end{center}
...
report. The repository carries 44 captures taken during development, including
the ones that record the failures, so every sweep, timing and recovery number
quoted in this report can be recomputed from committed data without a board.
...
malformed lines}. Zero drops is the load-bearing result: it is what shows that
emission never throttled acquisition, which is the premise every timing figure
rests on.

The acceptance gate for the release was that the four median acquisition
intervals be unchanged from the previous one, since the restructuring was
supposed to touch who may talk to the device and not the acquisition path. They
were identical in all three runs, and identical to the figures recorded for
1.5.0:
...
and 7~Pa in the worst of the three --- as it must be, since the true pressure
does not depend on how the sensor is sampled.

\paragraph{The measured history.} The gate above is a comparison against one
previous release, but every release was captured the same way, so the whole
development history can be read off the committed data. Twenty-two captures
recorded a complete sweep across eight firmware versions; the remaining
committed captures are deliberate failures --- wedged buses, resets landing
mid-transfer --- and are excluded here because a partial sweep is not comparable
with anything. One of the two 1.5.0 points is the forced bus-recovery build of
Section \ref{sec:problems} rather than the release binary; it is kept because
its four intervals are identical to the release's, which is precisely the check
that build exists to pass.
...
\item \textbf{1.2.1 to 1.3.0 --- 13 / 16 / 22 / 34~ms.} Two milliseconds
  ...
  while the sensor was still converting. This is the only step in the project
  where we paid measured throughput for correctness.
\item \textbf{1.4.0 onwards --- 5 / 7 / 11 / 19~ms.} Between 1.7 and 2.6 times
  ...
  grows with the mode --- 3~ms at \texttt{oss=0} against 10~ms at
  \texttt{oss=3} --- so it dominates the \emph{absolute} saving at the top mode,
  where 15 of the 34~ms disappear. The two changes are: the fixed
  worst-case sleep was replaced by polling the sensor's SCO bit, so a cycle ends
  when the conversion actually ends rather than at its datasheet maximum, and
  the temperature cache means most cycles skip the temperature conversion
  entirely. The padded maxima did not go away --- they became the timeout.
\end{itemize}

Release 2.0.0 lands exactly on the 1.4.0 line, which is the result the
restructuring had to produce: moving every consumer off the device and behind an
application layer was not allowed to cost the acquisition path anything, and it
did not.
...
Noise tells the opposite story, and that is also the correct one. Across all
eight versions each mode's per-version mean stays inside a band at most about
2~Pa wide, near its datasheet figure, with no trend: the firmware changes moved timing, structure
and fault behaviour, and none of them made the sensor quieter or noisier,
because noise is a property of the part and the oversampling setting. The two
eras are measured with different columns --- whole-block RMS before the
temperature cache existed, median within-segment RMS after it --- and the chart
marks the boundary rather than blending them, because with the cache off the
segments are two or three samples long and their spread understates the noise.
The one visible outlier, \texttt{oss=0} at 2.0.0, is the post-boot settling
transient identified below.
...
Oversampling demonstrably works --- noise falls by roughly a factor of two from
\texttt{oss=0} to \texttt{oss=2} in every run --- but we will not claim the
monotonicity criterion as passed. It holds across the first three modes in all
three runs; at the \texttt{oss=2} to \texttt{oss=3} step it holds in one run of
three.

We checked the obvious explanation and it does not survive. Since the sweep
visits each mode once in sequence, a block's exposure to real atmospheric
movement grows with the mode --- 2.5~s of wall clock at \texttt{oss=0} against
9.5~s at \texttt{oss=3} --- so we fitted and removed a linear pressure trend
from each block. At \texttt{oss=3} detrending changes almost nothing (3.22, 4.32
...
against a datasheet 6.0. The excess is a settling transient the sweep rides
through, not sensor noise --- which is an argument for discarding rather more
than three warm-up samples.

The structure of the profile is visible directly in the stream: the three short
sweep blocks, the long continuous run at \texttt{oss=2}, and the compensation
staircase produced by the one-second temperature cache.
...
as such. Pulling SDA during a run --- measured on the build immediately
preceding the current structure, and recorded in \texttt{TESTING.md} section 10;
that particular capture file is not among the committed ones --- produced
exactly one \texttt{E 5} (\texttt{EIO}) record 9\,995~$\mu$s after the last
sample, samples stopped rather than repeating with fresh timestamps, and the
stream resumed on reconnect with no reset, short by exactly the outage divided
by the cycle time (13\,088 samples against a 13\,663 baseline, zero drops).
...
boot --- a failure that on 1.4.0 killed three consecutive boots
...
The three things the project actually taught us:

\begin{itemize}
\item \textbf{How a real-time kernel's device model fits together.} What it
  means for an IMFS node to \emph{own} a device, why the teardown order is the
  whole safety argument, how an ioctl ABI is designed so it can be frozen, and
  how a bus driver and a device driver meet through a framework interface rather
  than through each other.
\item \textbf{That bringing up a peripheral means registers and nothing else.}
  Clock gating, alternate-function mux, CCR clock math --- and that an I2C bus
  has a failure mode, a slave holding SDA low, that no amount of retrying fixes
  and that has to be designed for explicitly.
\item \textbf{That lock-free is a scheduling decision before it is a performance
  ...
  priority-inheritance handoff completed. The same reasoning fixes the task
  priorities, and getting it wrong cost us a measured 2.70~s console blackout.
\end{itemize}

We also learned that \emph{on an embedded target the measurement is the test}.
...
the noise-versus-oversampling curve. A related lesson is that a datasheet is a
timing contract rather than a feature list: the conversion-time table is the
specification the driver has to honour, and one tick of rounding against it was a
real bug.
...
monotonically at the OSS1 to OSS2 step --- held in only one of five runs, and we
believe the sweep design is at fault rather than the driver: the modes are
visited once each in sequence, so slow atmospheric drift lands on them
unequally, and the datasheet difference between those two modes is 1~Pa against
run-to-run scatter of about $\pm 1$~Pa. Interleaving the modes within a sweep is
the change we would make before trusting that criterion.
...
\item \textbf{The BSP ships no I2C bus driver for the framework the device
  driver targets.} We discovered this only after the sensor driver was written:
  there was nothing for \texttt{i2c\_dev\_alloc\_and\_init()} to attach to. This
  roughly doubled the scope of the project --- the 622-line polled STM32F4
  master in \texttt{i2c/} exists entirely because of it.
...
\item \textbf{Every \texttt{open()} returned \texttt{ENFILE}.} The RTEMS default
  ...
  opening the bus node and the device node was impossible. The symptom (``too many open
  files'' on a system with two files) looked nothing like the cause.
...
\item \textbf{The console was silent for hours.} Two independent causes stacked:
  ...
  framing garbage --- which is why the capture tool applies them to an
  already-open descriptor and holds it for the whole run.
...
\item \textbf{Conversion waits expiring one tick early.}
  ...
  the result registers while the sensor was still converting --- and, crucially,
  while still within specification. The defect was stochastic and invisible in
  any single reading; what exposed it was that RMS noise did not fall
  monotonically with oversampling, which it must. The fix is one tick of padding
  on every conversion constant, and the header now says explicitly not to
  ``correct'' them back.
...
\item \textbf{The board came up dead after a reset, and stayed dead.} A CPU
  ...
  power cycle cleared it. This reproduces in roughly two boots out of seven once
  bus occupancy is high. The fix is the recovery sequence described in Section
  \ref{sec:design}.
...
\item \textbf{A failing sampler starved the task that was supposed to report the
  ...
  produced 2.70~s of complete console silence --- no samples, no errors, no
  drops --- because nothing below it could run. A fault that silences the
  reporting path is worse than the fault itself. The sampler now yields a tick
  after a failed cycle, and that yield is also the slot the consumer uses to
  notice and report the errno.
...
\item \textbf{A stale reading that looked fresh.} An early version stamped the
  ...
  Nothing in the data said anything was wrong. The layer now refuses to
  republish on a failed cycle, so the timestamp freezes and the age of the data
  stays honest --- and the header says so, because the check cannot be forced on
  the consumer.
...
\item \textbf{Aliasing between the temperature cache and the noise
  ...
  interval rather than sensor noise --- 16.21~Pa against 4.85~Pa within-segment
  in one measured block. The analysis now splits each block at the cache
  refreshes and reports the median within-segment RMS.
...
\item \textbf{A use-after-free in teardown.} Freeing the device directly leaves
  ...
  \texttt{unlink()}, which runs the node's destroy handler. Because nothing in a
  normal run ever tears the device down, the whole path was dead code --- which
  is exactly where the bug lived --- so it is now exercised under a build flag
  that registers, opens, unlinks and re-opens once at boot.
\end{itemize}
